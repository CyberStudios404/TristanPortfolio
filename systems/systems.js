const portfolioBase =
  "https://sites.google.com/view/tristanbernier/frameworks/";
...
const nodeMap = Object.fromEntries(
  nodes.map(n => [n.dataset.system, n])
);
...
function renderInspector(id) {
  ...
  document.getElementById("insp-status").innerHTML =
    `<span class="dot dot-${d.status}"></span>${d.statusLabel}`;
  ...
  const depsEl =
    document.getElementById("deps");

  const depsSec =
    document.getElementById("deps-section");
  ...
  const usedSec =
    document.getElementById("usedby-section");
...
function applyHighlight(id) {
  ...
  svg
    ...
    .forEach(l => {
      ...
      const keep =
        set.has(l.dataset.from) &&
        set.has(l.dataset.to);
...
function panTo(node) {
  ...
  const dx =
    (r.left + r.width / 2 -
      (m.left + m.width / 2)) * 0.025;

  const dy =
    (r.top + r.height / 2 -
      (m.top + m.height / 2)) * 0.025;


  map.style.transform =
    `scale(1.012) translate(${-dx}px,${-dy}px)`;
